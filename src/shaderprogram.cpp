...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      attrPos(-1), attrNor(-1), attrCol(-1), attrJointsIDs(-1), attrJointWeights(-1),
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    // Add the new in vecs for skeleton shader
    attrJointsIDs = context->glGetAttribLocation(prog, "vs_jointIDs");
    attrJointWeights = context->glGetAttribLocation(prog, "vs_weights");

//    std::cout << "jointIDs: " << attrJointsIDs << std::endl;
//    std::cout << "jointWeights: " << attrJointWeights << std::endl;

    unifModel      = context->glGetUniformLocation(prog, "u_Model");
    unifModelInvTr = context->glGetUniformLocation(prog, "u_ModelInvTr");
    unifViewProj   = context->glGetUniformLocation(prog, "u_ViewProj");
    unifCamPos      = context->glGetUniformLocation(prog, "u_CamPos");
    //std::cout << "unifCamPos: " << unifCamPos << std::endl;
    // Set handles for all the bind matrices and joints' overall tansformation matrices

    unifTransMats = context->glGetUniformLocation(prog, "u_TransMats");
    unifBindMats = context->glGetUniformLocation(prog, "u_BindMats");

//    std::cout << "unifTransMats: " << unifTransMats << std::endl;
//    std::cout << "unifBindMats: " << unifBindMats << std::endl;
}
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (attrJointsIDs != -1 && d.bindJointIDs()) {
        context->glEnableVertexAttribArray(attrJointsIDs);
        context->glVertexAttribIPointer(attrJointsIDs, 2, GL_INT, 0, nullptr);
    ...
    if (attrJointsIDs != -1) context->glDisableVertexAttribArray(attrJointsIDs);
