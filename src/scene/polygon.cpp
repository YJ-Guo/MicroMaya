...
void Polygon::Triangulate()
{
    ...
    // First only consider convex polygon
    for (int i = 0; i < m_verts.size() - 2; ++i) {
        // Use a temp triangle to store the mesh result
        // Only put in the index
        Triangle _triangle;
        _triangle.m_indices[0] = 0;
        _triangle.m_indices[1] = i + 1;
        _triangle.m_indices[2] = i + 2;

        // Add them to triangle list
        m_tris.push_back(_triangle);
    }
}

// Creates a polygon from the input list of MeshVertex positions and colors
    ...
    : m_tris(), m_verts(), m_name(name), mp_texture(nullptr), mp_normalMap(nullptr)
{
    ...
    {
        m_verts.push_back(MeshVertex(pos[i], col[i], glm::vec4(), glm::vec2()));
    }
    Triangulate();
}
    ...
    : m_tris(), m_verts(), m_name(name), mp_texture(nullptr), mp_normalMap(nullptr)
{
    ...
    for(int i = 0; i < sides; i++)
    {
        glm::vec4 vert_pos = glm::translate(glm::vec3(pos.x, pos.y, pos.z))
                ...
                * v;
        m_verts.push_back(MeshVertex(vert_pos, color, glm::vec4(), glm::vec2()));
    }
...
void Polygon::AddMeshVertex(const MeshVertex& v)
{
    m_verts.push_back(v);
}
...
MeshVertex &Polygon::VertAt(unsigned int i)
{
    return m_verts[i];
}

MeshVertex Polygon::VertAt(unsigned int i) const
{
    return m_verts[i];
}
